...
class BinaryTree {
    ...
    public boolean isBinarySearchTree(Node root) {
        ...
        if (root.data >= root.left.data || root.data <= root.right.data) {// if the part is less than the left or right data
            return false; //its flase
        }
    ...
    public Node build_from_node_lists(int[] pre, int[] in, int start, int end) {//index starts at 0
        //for int array lists
        int preIndex = 0;
        if (start > end) {
            return null;
        ...
        int inOrderIndex = getInOrderIndex(newNode.data, in, start, end);
        newNode.left = build_from_node_lists(pre, in,  start,inOrderIndex - 1);
        newNode.right = build_from_node_lists(pre, in, inOrderIndex + 1,end);
        return newNode;
    }

    public int getInOrderIndex(int val, int in[], int start, int end) {//findinf inorder Index with int arrays
       int result=-1;
    ...
    public Node build_from_node_lists(double[] pre, double[] in, int start, int end) {//for double array lists
        //binary search tree
        int preIndex = 0;
        if (start > end) {
            return null;
        ...
        int inOrderIndex = getInOrderIndexD(newNode.data, in, start, end);
        newNode.left = build_from_node_lists(pre, in, start, inOrderIndex - 1);
        newNode.right = build_from_node_lists(pre, in,inOrderIndex + 1,end);
        return newNode;
    }
    public int getInOrderIndexD(int val, double[] in, int start, int end) {//findinf inorder Index with double arrays
        int result=-1;
        for (int i = start; i <= end; i++) {
            if (in[i] == val) {
                return i;
    ...
    public int depth(Node node) {//find depth in trees
        if (node == null) {
            return -1;
        } else {
            int leftDepth = depth(node.left);//find depth of left nodes
            int rightDepth = depth(node.right);//finds depth of right node
    ...
    public double max(Node nodeX) {//maxDouble if finding the max for the DOuble
        if (nodeX == null) {
            return -1;
        }
        double rightSideData = max(nodeX.right);//keeps going to the right until it reaches rightest Node
        double leftSideDate = max(nodeX.left);//keepts goiing to left until it reaches leftest Node
        double dataX = nodeX.data;

        if (dataX < leftSideDate) {//checks if the data for the left side is greater than the data form the node
            dataX = leftSideDate;//if so then make the dataNode for the max variable the leftsideData
        }
        if (dataX < rightSideData) {//checks if the daa for the right side is greate than the data from the node data
            dataX = rightSideData;//if so then make the right side data the max Data
        }
        return dataX;
    }

    public double tree_sum(Node root) {//sums nodes in array
        if (root == null) {
            return 0.0;
        }
    ...
    public double tree_average(Node root) {//gets avg of the nodes in array
        return tree_sum(root) / arraySize;

    }

    public boolean tree_is_balanced(Node root) {
        if (heightOfTree(root) == 0) {
            return true;
        }
        return false;
    }
    ...
    int heightOfTree(Node root) {
        if (root == null) {
            return 0;//true balanced
        }

        int leftHeight = heightOfTree(root.left);//find higeht of left side of tree
        if (leftHeight == -1) {
            return -1;//false not balances
        }
        int rightHeight = heightOfTree(root.right);//finds hight of right side of tree
        if (rightHeight == -1) {
            return -1;
        }

        if (Math.abs(leftHeight - rightHeight) > 1) {//if the difference in depths between the left and right subtrees
...
public class BinaryTreeX {//I tired my best but it still was not working

    public static void main(String[] args) {//I was not bale to get my code to work but I trie
        BinaryTree binaryTree = new BinaryTree();//binary tree 1
        int pre[] = {2, 1, 3};   // pre-order
        int in[] = {1, 2, 3};   // in-order
        binaryTree.build_from_node_lists(pre, in, in.length - 1,0);
        System.out.println("Tree name: Tree 1");
        System.out.println("Binary Search Tree: " + binaryTree.isBinarySearchTree(binaryTree.root));
        System.out.println("Depth: " + binaryTree.depth(binaryTree.root));
        System.out.println("Max: " + binaryTree.max(binaryTree.root));
        System.out.println("Sum: " + binaryTree.tree_sum(binaryTree.root));
        System.out.println("Average: " + binaryTree.tree_average(binaryTree.root));
        System.out.println("Is Balanced: " + binaryTree.tree_is_balanced(binaryTree.root));

        binaryTree = new BinaryTree();//binary tree 2
        int[] pre2 = {1, 2, 4, 8, 5, 9, 3, 6, 10, 11, 7, 12};   // pre order
        int[] in2 = {8, 4, 2, 9, 5, 1, 10, 6, 11, 3, 7, 12};  // in order
        binaryTree.build_from_node_lists(pre2, in2, in2.length - 1,0);//
        System.out.println("Tree name: Tree 2");
        System.out.println("Binary Search Tree: " + binaryTree.isBinarySearchTree(binaryTree.root));
        System.out.println("Depth: " + binaryTree.depth(binaryTree.root));
        System.out.println("Max: " + binaryTree.max(binaryTree.root));
        System.out.println("Sum: " + binaryTree.tree_sum(binaryTree.root));
        ...
        binaryTree = new BinaryTree();//binary tree 3
        ...
        binaryTree.build_from_node_lists(pre3, in3, in3.length - 1,0);
        System.out.println("Tree name: Tree 3");
        System.out.println("Binary Search Tree: " + binaryTree.isBinarySearchTree(binaryTree.root));
        System.out.println("Depth: " + binaryTree.depth(binaryTree.root));
        System.out.println("Max: " + binaryTree.max(binaryTree.root));
        System.out.println("Sum: " + binaryTree.tree_sum(binaryTree.root));
        ...
        binaryTree = new BinaryTree();//tree 4
        ...
        binaryTree.build_from_node_lists(pre4, in4, in4.length - 1,0);
        System.out.println("Tree name: Tree 4");
        System.out.println("Binary Search Tree: " + binaryTree.isBinarySearchTree(binaryTree.root));
        System.out.println("Depth: " + binaryTree.depth(binaryTree.root));
        System.out.println("Max: " + binaryTree.max(binaryTree.root));
        System.out.println("Sum: " + binaryTree.tree_sum(binaryTree.root));
        ...
        binaryTree = new BinaryTree();//tree 5
        ...
        binaryTree.build_from_node_lists(pre5, in5, in5.length - 1,0);
        System.out.println("Tree name: Tree 5");
        System.out.println("Binary Search Tree: " + binaryTree.isBinarySearchTree(binaryTree.root));
        System.out.println("Depth: " + binaryTree.depth(binaryTree.root));
        System.out.println("Max: " + binaryTree.max(binaryTree.root));
        System.out.println("Sum: " + binaryTree.tree_sum(binaryTree.root));
        ...
        binaryTree = new BinaryTree();//tree 6
        ...
        binaryTree.build_from_node_lists(pre6, in6, in6.length - 1,0);
        System.out.println("Tree name: Tree 6");
        System.out.println("Binary Search Tree: " + binaryTree.isBinarySearchTree(binaryTree.root));
        System.out.println("Depth: " + binaryTree.depth(binaryTree.root));
        System.out.println("Max: " + binaryTree.max(binaryTree.root));
        System.out.println("Sum: " + binaryTree.tree_sum(binaryTree.root));
...
class Node {
    int data;//stores int data from int arrays
    double dData;//stores double data from double arrays
    Node right, left;

  /*  Node(int data) {//int Node
        this.data = data;
        this.right = null;
        this.left = null;
    }*/

    Node(double data) {//double Node
        this.dData = data;// Am i supposed to this??
        this.right = null;
        this.left = null;
    }
}